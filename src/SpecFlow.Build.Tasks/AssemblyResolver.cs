	...
	internal static class AssemblyResolver
	{
		...
		private static Assembly CurrentDomain_AssemblyResolve(Object sender, ResolveEventArgs args)
		{
			...
			if (!String.IsNullOrEmpty(assemblyPath))
			{
				...
				Console.WriteLine($"Considering {probingPath} based on RequestingAssembly");
			...
			Console.WriteLine($"Considering {probingPath} based on BaseDirectory");
			if (Probe(probingPath, referenceName.Version, out assm))
			{
				return assm;
			}
			...
			if (!String.IsNullOrEmpty(assemblyPath))
			{
				...
				Console.WriteLine($"Considering {probingPath} based on ExecutingAssembly");
				if (Probe(probingPath, referenceName.Version, out assm))
				{
					return assm;
				}
			...
			Console.WriteLine($"Considering {fileName}");
			if (Probe(fileName, referenceName.Version, out assm))
			{
				return assm;
			}

			// look next to the entry assembly
			assemblyPath = Assembly.GetEntryAssembly().Location;
			if (!String.IsNullOrEmpty(assemblyPath))
			{
				probingPath = Path.Combine(Path.GetDirectoryName(assemblyPath), fileName);

				Console.WriteLine($"Considering {probingPath} based on EntryAssembly");
				if (Probe(probingPath, referenceName.Version, out assm))
				{
					return assm;
				}
		...
		private static Boolean Probe(String filePath, Version minimumVersion, out Assembly assembly)
		{
